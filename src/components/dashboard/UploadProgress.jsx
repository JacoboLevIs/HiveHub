...
export default function UploadProgress({ completedSinceLastUpload = 0 }) {
  const required = 3;
  const progress = Math.min(completedSinceLastUpload, required);
  const percentage = (progress / required) * 100;
  const unlocked = progress >= required;
    ...
    <div className="bg-card rounded-2xl border border-border p-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-lg font-semibold mt-0.5">
            {unlocked ? 'Ready to upload!' : `${progress} / ${required} tests completed`}
          </p>
        </div>
      ...
      <p className="text-xs text-muted-foreground mt-2">
        {unlocked
          ...
          : `Complete ${required - progress} more test${required - progress !== 1 ? 's' : ''} to unlock`
        }
      </p>
    </div>
  );
